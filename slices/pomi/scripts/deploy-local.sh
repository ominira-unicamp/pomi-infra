...
playbook_args=(playbooks/deploy-pomi.yml)
if [[ "${USE_EXISTING_IMAGE:-false}" == "true" ]]; then
  [[ -n "${IMAGE_TAG:-}" ]] || {
    echo "IMAGE_TAG é obrigatório com USE_EXISTING_IMAGE=true." >&2
    exit 1
  }
  playbook_args+=(-e pomi_use_existing_image=true -e "pomi_image_tag=$IMAGE_TAG")
fi
if [[ -n "${POMI_CORS_ORIGIN:-}" ]]; then
  playbook_args+=(-e "pomi_cors_origin=$POMI_CORS_ORIGIN")
fi
